...
class Species:
    ...
    def __init__(self, name: SpeciesName, mass: float, radius: float, color: Color) -> None:
...
class Particle:
    ...
    def __init__(
        self,
        species: Species,
        pos: Union[List[float], Tuple[float, float], NDArray[np.float64]],
        vel: Union[List[float], Tuple[float, float], NDArray[np.float64]],
    ) -> None:
...
def particle_simulator_initial_steps(
    Matrix_A: Union[List[float], Tuple[float, float, float], NDArray[np.float64]],
    Matrix_B: Union[List[float], Tuple[float, float, float], NDArray[np.float64]],
    Matrix_C: Union[List[float], Tuple[float, float, float], NDArray[np.float64]],
) -> List[Particle]:
    particles: List[Particle] = []
...
def setup_plot(
    ...
    speed_hist: Histogram = Histogram(speeds, xmax=np.max(speeds) * 2, nbars=50, density=True)
...
def particle_simulator(
    ...
    """
    particles: List[Particle] = particle_simulator_initial_steps(Matrix_A, Matrix_B, Matrix_C)
