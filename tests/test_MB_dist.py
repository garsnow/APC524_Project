...
def is_maxwell_boltzmann(speeds: np.ndarray, masses: np.ndarray, T: float) -> bool:
    kb: float = 1.38e-23
    cdf = lambda v: sum([(2 / np.sqrt(np.pi)) * (np.sqrt(m / (2 * kb * T)) ** 3) * v ** 2 * np.exp(-m * v ** 2 / (2 * kb * T)) for m in masses])
...
def setup_simulation():
    ...
    dt = 1 / 30  # Time step
    return sim, dt, np.array([p.mass for p in particles]), 300  # Return the simulation, time step, masses, and temperature
...
def test_MB():
    ...
    assert is_maxwell_boltzmann(final_speeds, masses, T), "Final speed distribution does not match Maxwell-Boltzmann distribution"
